...
class RoIHeads(torch.nn.Module):
    ...
    def postprocess_detections(self, class_logits, box_regression, proposals, image_shapes):
        ...
        for boxes, scores, image_shape in zip(pred_boxes, pred_scores, image_shapes):
            ...
            temp_score_thresh = self.score_thresh
            while len(inds) < 36:
                temp_score_thresh -= 0.01
                inds = torch.nonzero(scores >= temp_score_thresh).squeeze(1)
            ...
            for i in range(num_images):
                result.append(
                    dict(
                        ...
                        labels=labels[i],
                        scores=scores[i],
                        boxes_feature=self.box_roi_pool(features, [boxes[i]], image_shapes)
                    )
                )
